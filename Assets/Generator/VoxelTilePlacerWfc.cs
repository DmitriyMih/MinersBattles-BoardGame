...
public class VoxelTilePlacerWfc : MonoBehaviour
{
    ...
    public Transform stortage;
    public Transform cloneStortage;
    ...
    public void Awake()
    {
        tileMapSizeX = MapSize.x - 2;
        tileMapSizeY = MapSize.y - 2; ;
    }

    private void Start()
    {
        spawnedTiles = new VoxelTile[MapSize.x, MapSize.y];
        GlobalGridController.globalGridController.gridMap = new Grid[MapSize.x - 2, MapSize.y - 2];
        ...
        {
            VoxelTile clone; Grid currentGrid;
            switch (TilePrefabs[i].Rotation)
            {
                ...
                case VoxelTile.RotationType.TwoRotations:
                    ...
                    clone = Instantiate(TilePrefabs[i], TilePrefabs[i].transform.position + Vector3.right,
                        Quaternion.identity); clone.transform.parent = cloneStortage;
                    clone.Rotate90();
                    //
                    currentGrid = clone.gameObject.GetComponent<Grid>();
                    currentGrid.gridRotation = clone.transform.eulerAngles.y;
    ...
    private void PlaceAllTiles()
    {
        ...
        stortage.localScale *= 8f;
        cloneStortage.gameObject.SetActive(false);
    }
    ...
    private void PlaceTile(int x, int y)
    {
        ...
        //
        Grid currentGrid = spawnedTiles[x, y].gameObject.GetComponent<Grid>();
        currentGrid.currentCoordinat = new Vector2Int(x, y);
        GlobalGridController.globalGridController.gridMap[x - 1, y - 1] = currentGrid;
        //
        foreach (VoxelTile spawnedTile in spawnedTiles)
        {
            if (spawnedTile != null)
                spawnedTile.transform.parent = stortage;
